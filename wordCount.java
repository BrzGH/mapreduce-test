...
//        }
    ...
    static class WCCombiner extends Reducer<Text, LongWritable, Text, LongWritable>{
        @Override
        protected void reduce(Text key, Iterable<LongWritable> values, Context context) throws IOException, InterruptedException {
            //为什么用迭代器
            /**
...
//        conf.set("dfs.client.failover.proxy.provider.qianfeng","org.apache.hadoop.hdfs.server.namenode.ha.ConfiguredFailoverProxyProvider");
       ...
       // job.setNumReduceTasks(3);
        ...
        //10、设置Job的输入文件的路径
        FileInputFormat.setInputPaths(job,new Path("F:\\hdfs\\worldcount\\test\\Test1.java"));

        //11、设置Job的输出文件的路径
        FileOutputFormat.setOutputPath(job,new Path("F:\\hdfs\\worldcount\\test\\output"));
